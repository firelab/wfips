...
/*
// FOR FARSITE v4.0 not FlamMap2.0
void MechCalls::GetPoints(long CurrentFire, long CurrentPoint)
{
	long NumPts;

     NumPts=GetNumPoints(CurrentFire)-1;

	if(CurrentPoint==0)
	{    xpt=GetPerimeter2Value(0, XCOORD);
		ypt=GetPerimeter2Value(0, YCOORD);
		xptl=GetPerimeter2Value(NumPts, XCOORD);
		yptl=GetPerimeter2Value(NumPts, YCOORD);
	}
	else
	{    xpt=GetPerimeter2Value(CurrentPoint, XCOORD);//xptn;
     	ypt=GetPerimeter2Value(CurrentPoint, YCOORD);//yptn;
		xptl=GetPerimeter2Value(CurrentPoint-1, XCOORD);
		yptl=GetPerimeter2Value(CurrentPoint-1, YCOORD);
	}
	if(CurrentPoint!=NumPts)
	{    xptn=GetPerimeter2Value(CurrentPoint+1, XCOORD);
		yptn=GetPerimeter2Value(CurrentPoint+1, YCOORD);
	}
	else
...
void MechCalls::GetPoints(long row, long col)
{// need to reconstruct the points and orientations of each slope facet
	//xdiff=GetCellResolutionX();

     xpt=col*GetCellResolutionX()+GetLoEast()+GetCellResolutionX()/2.0;;
     ypt=GetHiNorth()-row*GetCellResolutionY()-GetCellResolutionY()/2.0;
}
...
void MechCalls::CalculateFireOrientation(double DegreesFromMaxSpread)
{
	//double tangent;
     ...
     if(GetOutputDirection()==RELATIVEDIR)
...
{// does first acceleration of fire, computes Ros at T+1 given time step
	...
	if(RosT<vecros && AccelerationON()) 	// if RosT is < equilibrium, then accelerate
...
{// does second acceleration of fire, computes Ros at T+1 given distance traveled
	...
	{	if(RosT<vecros && AccelerationON() && ExpansionRate>0.0)  // if not at steady state ROS
    ...
    /* Forcing time to 0.0, this is due to an issue in */
    //timerem = 0.0;
	if(timerem<0.0)
	{	fli*=-1.0;
		FliFinal*=-1.0;
...
{// does first acceleration of fire, computes Ros at T+1 given time step
	if(RosT<cros && AccelerationON())			   	// if RosT is < equilibrium, then accelerate
...
{// does second acceleration of fire, computes Ros at T+1 given distance traveled
	if(timerem>0.0)                  	 // if time remaining, e.g. no limits to growth from distcheck (limgrow())
	{	if(RosT<cros && AccelerationON() && ExpansionRate>0.0)				 // if not at steady state ROS
		{    timerem=SubTimeStep;      // reset timerem-- don't use limgrow() to decrement timestep, use accel()
			RosT1=accel(cros, RosT, A, &avgros, &cosslope);
...
{// calls slope correction for spread rate
	...
	{    ActualSurfaceSpread=sqrt(ActualSurfaceSpread);
		...
		xpt=midx-(timerem*xt*RosReduct*MetricResolutionConvert()); // computes new horizontal xpt
		ypt=midy-(timerem*yt*RosReduct*MetricResolutionConvert()); // and ypt coordinates
	}
}
...
{// calculates distances around fires and resets distance checking
	...
	{	xpt=GetPerimeter2Value(0, XCOORD);
		...
		{	if(i<NumPtTotal)
          ...
          if(NumPtActive>1)
     		NewMinDist=(ActivePerimDist/(double) NumPtActive)/MetricResolutionConvert();		// convert to meters
          else
               NewMinDist=-1.0;
...
{// limits distance growth to distance check and decrements remaining time step
	...
	{    fdist=MINDIST/dist;
		...
		HorizSpread=GetDynamicDistRes();	// horizontal spread now equals the distance check
	}
	else
		timerem=0.0;					// no time in timestep remaining
	if(xpt<=GetLoEast())    				// keeps fire on visible landscape
	{	xpt=midx;                          // keep old point to prevent coincident points on edge of landscape
		timerem=-1.0;
	}
	else if(xpt>=GetHiEast())
	{	xpt=midx;
		timerem=-1.0;
	}
	if(ypt<=GetLoNorth())
...
{// ROS acceleration from point source,VanWagner's (McAlpine and Wakimoto 1991)
	...
	{    double t, f1, f2, B=RosE/A;
		double LimSpread=GetDynamicDistRes()/(*cosslope);	// convert horizontal distance check to slope distance equivalent
...
{// Rothermel spread equation based on BEHAVE source code,
 //  support for dynamic fuel models added 10/13/2004
	...
	double rir[2]={0,0};
     ...
     if(!GetNewFuel(fuel, &newfuel)) // will get all models, 13, 40, and custom
     {	rateo=0.0;
