...
void Embers::SetFireEnvironmentCalls(FireEnvironment2 *Env, FELocalSite *Fe)
{
	env=Env;
	fe=Fe;
}
...
void Embers::Plume(double CFlameLength, double CFBurned)
{
	...
	{	long treespecies=GetCanopySpecies();
		long tnum=1;     				//tnum= # trees simultaneously torching in group
		double DBH=GetAverageDBH()/2.54;   // cm to inches

		xdiffl=(Fcoord.x-Fcoord.xl)/2.0/MetricResolutionConvert();   // required to be in meters
		ydiffl=(Fcoord.y-Fcoord.yl)/2.0/MetricResolutionConvert();
		xdiffn=(Fcoord.x-Fcoord.xn)/2.0/MetricResolutionConvert();
		ydiffn=(Fcoord.y-Fcoord.yn)/2.0/MetricResolutionConvert();
...
{// Iterates ember flight down through logarithmic wind profiles
	...
	long IgProb=PercentIgnition(GETVAL)*100;		// in FSXW.CPP
	long ProbSpot;
	...
	{    CurEmber=&(FirstEmber[count]);
		...
		{	posit=fe->GetLandscapeData(seast, snorth, mech.ld);
			...
			if(eH<=0.0)
				eH=1.0;
			fe->GetFireEnvironment(env, CurTime, false);//, 0, 1, -1);
		...
		{	ztest1=voo*tao*((elapsedtime*60.0)/tao-.5*pow2((elapsedtime*60.0)/tao));// compute test vertical drop of ember over sstep (0.25 min)
			...
			{    positlast=posit;							// file position of raster
					...
					{	UH=VertWindSpeed(Z1AboveGround, eH)/2.0;	// half of UeH in ft/s
						...
						{	if(Z2AboveGround>0)
							...
							{	dxdt=UH/2.03*log(ZAvgAboveGround/(.1313*eH));		// dxdt is ft/s
								...
								mXt=0.0;
							if(PassB<1 && mXt>(GetCellResolutionX()/MetricResolutionConvert()-0.5))
				...
				if(PassA>7)
					break;
				tseast=seast-mXt*MetricResolutionConvert()*sin(rwinddir);		// temporary position change, will be permanent if Z iterations ok
				tsnorth=snorth-mXt*MetricResolutionConvert()*cos(rwinddir);
				if(tseast>=GetHiEast() || tseast<=GetLoEast() || tsnorth>=GetHiNorth() || tsnorth<=GetLoNorth())
