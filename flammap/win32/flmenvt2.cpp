...
long FECalls::GetLandscapeData(double xpt, double ypt, LandscapeStruct& ls)
{
...
void LandscapeData::HeightConvert(short height)
{
     if(HaveCrownFuels())
     {	if(height>=0)
		{    short units=GetTheme_Units(H_DATA);
			...
			if(ld.height>100.0)
          	...
          	ld.height=GetDefaultCrownHeight();
     }
    	else
         	ld.height=GetDefaultCrownHeight();
}
...
void LandscapeData::BaseConvert(short base)
{
     if(HaveCrownFuels())
     {	if(base>=0)
		{    short units=GetTheme_Units(B_DATA);
...
void LandscapeData::DensityConvert(short density)
{
     if(HaveCrownFuels())
     {	if(density>=0)
		{    short units=GetTheme_Units(P_DATA);
			...
			if(ld.density>1.0)
          	...
          	ld.density=GetDefaultCrownBD(ld.cover);
     }
    	else
         	ld.density=GetDefaultCrownBD(ld.cover);
}
...
void FECalls::GetFireEnvironment(double SIMTIME, int type, long ReadNewData, long PointNum)
{
     long oldfuel=ld.fuel;
...
void FECalls::globaldata(long ReadNewData, long PointNum)
{
	if(ReadNewData)
	{	gmw.ones=ohmc/100;
		gmw.tens=thmc/100;
...
long FECalls::AllocFireMW(long NumPoints)
{
	if(!FireMW)
...
void FireEnvironment::envt(double SIMTIME)
{
	...
	long elevref,  cloud=0;
     long AtmGridExists=AtmosphereGridExists();
     double LagRate;
	...
	if(hour>=1400)
		...
		lastdate=date-1;
	curhour=1400;
	thmc=GetInitialFuelMoisture(ld.fuel, 1);//FuelMoisture.TL10[fuel-1];
	hhmc=GetInitialFuelMoisture(ld.fuel, 2);//FuelMoisture.TL100[fuel-1];
	for(curdate=GetStartDate(); curdate<=lastdate; curdate++)
	{    thisdate=curdate;
     	if(thisdate>365)
          	thisdate-=365;
...
//			rain/=100.0;
	...
	ohmc=(4.0*xemc+thmc)/5.0;     /* one hour fuel mc */
     ...
     {    LagRate=1.0-1.0/exp(SIMTIME/60.0);
          ...
          ohmc=GetInitialFuelMoisture(ld.fuel, 1)-LagRate*(GetInitialFuelMoisture(ld.fuel, 1)-ohmc);
     }
	FuelTemp=temp;
//	PtTemperature=temp;
//	PtHumidity=humid;
...
{
	long count=-1;
     long ddate, month, day, hhour, xmonth, xday;
	...
	{	tempmin1=-1;
		...
		{    xmonth=GetAtmosphereGrid()->GetAtmMonth(count);
			...
			if(xday>day || xmonth>month)
               ...
               {	GetAtmosphereGrid()->GetAtmosphereValue(ATMTEMP, XLocation, YLocation, count, &tempmin2);
	     	     GetAtmosphereGrid()->GetAtmosphereValue(ATMHUMID, XLocation, YLocation, count, &humidmin2);
                    if(tempmin1==-1)
                    {	tempmin1=tempmin2;
                    	humidmin1=humidmin2;
                    }
          ...
          --count;
          GetAtmosphereGrid()->GetAtmosphereValue(ATMTEMP, XLocation, YLocation, count, &tempx);
          GetAtmosphereGrid()->GetAtmosphereValue(ATMHUMID, XLocation, YLocation, count, &humidx);
          GetAtmosphereGrid()->GetAtmosphereValue(ATMRAIN, XLocation, YLocation, count, &ppt);
          *temp=(double) tempx;
          *humid=(double) humidx;
...
void FireEnvironment::AtmWindAdjustments(int curdate, int hour, long *cloud)
{
...
void FireEnvironment::humtemp(int date, int hour, double *tempref, double *humref, long *elevref, double *rain, double *humidmx, double *humidmn, double *tempmx, double *tempmn)
/* FINDS AND INTERPOLATES HUMIDTY AND TEMPERATURE FOR CURRENT HOUR */
{
	long count=-1, ddate=-1, month, day, hmorn, haft, Tmin, Tmax, Hmax, Hmin;
...
//	double dewptref, dewpt;
	...
	elref=GetWeatherElev(StationNumber, count);
	if(hour>haft)
	{	count++;
...
void FireEnvironment::sitespec(long elevref, long elevx, double tempref, double *temp, double humref, double *humid)
...
void FireEnvironment::windadj(int date, int hour, long *cloud)
...
double FireEnvironment::irradiance(long cloud, int date, int hour, double *shade)
