...
#define FUEL_BAND 4
#define COVER_BAND 5
#define HEIGHT_BAND 6
#define BASE_BAND 7
#define DENSITY_BAND 8
...
int main( int argc, char *argv[] )
{
    ...
    pszInputFilename = CPLFormFilename(pszDataPath, "autotest/fbtest_crown", ".lcp");
    GDALDatasetH hLcpDS = GDALOpen(pszInputFilename, GA_ReadOnly);
    pszInputFilename = CPLFormFilename(pszDataPath, "autotest/fbtest_crown", ".ROS");
    GDALDatasetH hRosDS = GDALOpen(pszInputFilename, GA_ReadOnly);
    pszInputFilename = CPLFormFilename(pszDataPath, "autotest/fbtest_crown", ".FLI");
    GDALDatasetH hFliDS = GDALOpen(pszInputFilename, GA_ReadOnly);
    pszInputFilename = CPLFormFilename(pszDataPath, "autotest/fbtest_crown", ".FML");
    ...
    GDALRasterBandH hElevBand, hSlopeBand, hAspectBand, hFuelBand, hCoverBand,
                    hHeightBand, hBaseBand, hDensityBand;
    ...
    hHeightBand = GDALGetRasterBand(hLcpDS, HEIGHT_BAND);
    hBaseBand = GDALGetRasterBand(hLcpDS, BASE_BAND);
    hDensityBand = GDALGetRasterBand(hLcpDS, DENSITY_BAND);
    ...
    short nElev, nSlope, nAspect, nFuel, nCover, nHeight, nBase, nDensity;
    ...
    for(int i = 0;i < nXSize; i++)
    {
        ...
        {    
            ...
            GDALRasterIO(hCoverBand, GF_Read, i, j, 1, 1, &nCover, 1, 1, 
                         GDT_Int16, 0, 0);
            GDALRasterIO(hHeightBand, GF_Read, i, j, 1, 1, &nHeight, 1, 1, 
                         GDT_Int16, 0, 0);
            GDALRasterIO(hBaseBand, GF_Read, i, j, 1, 1, &nBase, 1, 1, 
                         GDT_Int16, 0, 0);
            GDALRasterIO(hDensityBand, GF_Read, i, j, 1, 1, &nDensity, 1, 1, 
                         GDT_Int16, 0, 0);
            ...
            lcp_data.height = nHeight;
            lcp_data.base = nBase;
            lcp_data.density = nDensity;
            lcp_data.duff = 0;
            lcp_data.woody = 0;
