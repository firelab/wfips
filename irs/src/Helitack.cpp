// Helitack.cpp
// Function definitions for helitack class CHelitack derived from CAirDelivered derived from CResource
// for IRS 1/12
...
#include "DispLoc.h"
// Include definition for class CHelitack from Helitack.h
#include "Helitack.h"
...
CHelitack::CHelitack( string rescID, CRescType &resctype, int staffing, string starttime, string endtime, string startdayofweek, 
...
// Copy constructor for CHelitack
CHelitack::CHelitack( const CHelitack &helitack ) : CAirDelivered( helitack )
{}
...
int CHelitack::CalcInitArrival( CFire fire, double distance )
{
	...
	// Add Walkin delay - Helitack always use the walk in delay
	...
	if ( available > 0 )	{										//resource is the first fire of the year
		...
		if ( difference > 0 )	{
			...
			//cout << "minutes since midnight for resource: " << availminmidnight << " difference: " << difference << "\n";
			//cout << " Competition Delay: " << difference << "\n";
		}
	} 
	//cout << "Initial arrival time: " << initarrivaldelay << "\n";
...
bool CHelitack::IsRescAvailable( CFire fire)
{
...
// Get the fireline production rate for the Helitack/Rappel crews
double CHelitack::DetermineProdRate( CFire fire, std::vector< CProdRates > &VProdRates, int timeMSM ) 
{		
	...
	int i = 0;
	while ( ProdRate < 0 && i < static_cast< int >(VProdRates.size()) )	{
...
int CHelitack::CreateContainValues( CFire fire, int firstarrival, std::vector< CProdRates >  &VProdRate, std::list< CContainValues > &LContainValues, double Multiplier )
{
