...
public:
	...
	CDispLoc( string ID, int callbackdelay, std::vector< CFWA* > AssocFWAs, string FPU = "" , double Latitude = 0, double Longitude = 0  );		//constructor for resource type
	...
	CDispLoc& operator=( const CDispLoc &rhs );		//assignment operator
	...
	void SetAssocFWAs( std::vector< CFWA* > AssocFWAs );	//set all the associated FWAs
	...
	std::vector< CFWA* > GetAssocFWAs();			//get a list with the associated FWA 
	...
	std::vector< CFWA* > m_AssocFWAs;				//list of associated FWAs
